...
]  # Specifies the data format of input to JASMINETrainer.compute_losses


@logged
class JASMINETrainer(Trainer):
    ...
    def __init__(
            ...
            latent_dim: int = None, lam_poe: float = None, lam_klcross: float = None,
            lam_contrastive: float = None, temperature: float = None,
            lam_contrast_samp: float = None, perturb_prop: float = None,
            lam_orthog: float = None,
        ...
        self.required_losses = [
            "dsc_loss", "gen_loss","x_elbo_sum",
            "joint_cross_loss", "real_cross_loss",
            "cos_loss", "klcross_loss", "x_elbo_sum_POE",
            "ctLoss", "ctLoss_samp", "orthog_loss"
        ]
        if self.net.u2c:
        ...
        self.lam_klcross = lam_klcross
        self.lam_poe = lam_poe
        self.lam_contrastive = lam_contrastive
        self.lam_contrast_samp = lam_contrast_samp
    ...
    def compute_losses(
        ...
        # Getting embeddings for perturbed samples
        x_aug, xrep_aug = perturb_samp(x, xrep, net, self.perturb_prop)

        u, l = {}, {}
        for k in net.keys:
            u[k], l[k] = net.x2u[k](x_aug[k], xrep_aug[k], lazy_normalizer=dsc_only)
        ...
        # PoE-derived joint distribution
        batch_size = xrep[net.keys[0]].shape[0]
        batch_size_aug = xrep_aug[net.keys[0]].shape[0]
        n_mod = len(net.keys)
        mu = torch.zeros(n_mod+1, batch_size_aug, self.latent_dim, device=self.net.device)
        sigma = torch.ones(n_mod+1, batch_size_aug, self.latent_dim, device=self.net.device)
        for i in range(0,len(net.keys)):
            tempmean = u[net.keys[i]].mean # batch_size_aug x latent_dim*n_mod
            tempmean = tempmean.reshape(batch_size_aug, self.latent_dim, n_mod) # batch_size_aug x latent_dim x n_mod
            tempmean = tempmean.permute(2,0,1) # n_mod x batch_size_aug x latent_dim
            tempmean = tempmean[i,:,:]
            mu[i+1,:,:] = tempmean

            tempsigma = u[net.keys[i]].stddev
            tempsigma = tempsigma.reshape(batch_size_aug, self.latent_dim, n_mod)
            tempsigma = tempsigma.permute(2,0,1)
            tempsigma = tempsigma[i,:,:]
            sigma[i+1,:,:] = tempsigma

        pmsk_orig = pmsk.clone()
        pmsk = torch.cat((pmsk,pmsk),dim=0)
        onesForPrior = torch.ones((pmsk.shape[0], 1), device=self.net.device)
        pmsk = torch.cat((onesForPrior, pmsk), dim=1)
        pmsk = pmsk.T
        pmsk_stack = pmsk.unsqueeze(2).expand_as(mu)
        eps=1e-8
        var = torch.square(sigma) + eps
        T = 1. / (var + eps)
        pd_mu = torch.sum(mu * T * pmsk_stack, dim=0) / torch.sum(T * pmsk_stack, dim=0)
        pd_var = 1. / torch.sum(T * pmsk_stack, dim=0)
        pd_std = torch.sqrt(pd_var)
        uPOE = D.Normal(pd_mu, pd_std)
        # Sampling from joint distribution
        uPOEsamp_aug = uPOE.rsample()

        if not self.lam_poe:
            pmsk = torch.cat((pmsk_orig,pmsk_orig),dim=0)
            pmsk = pmsk.T
            usamp_stack = torch.stack([usamp[k] for k in net.keys]) # n_mod x batch_size x n_mod*n_feat
            pmsk_stack = pmsk.unsqueeze(2).expand_as(usamp_stack)
            usamp_mean = (usamp_stack * pmsk_stack).sum(dim=0) / pmsk_stack.sum(dim=0) # batch_size x n_mod*n_feat
            if self.normalize_u:
                usamp_mean = F.normalize(usamp_mean, dim=1)
            usamp_mean_split = torch.stack(torch.tensor_split(
                usamp_mean, len(net.keys), dim=1
            )) # n_mod x batch_size x n_feat
            uPOEsamp_aug = usamp_mean_split.mean(axis=0)

        # Also taking uPOEsamp for original data (frist half of batch)
        uPOEsamp = uPOEsamp_aug[:batch_size,:]
        ...
        if self.lam_poe:
            ...
            x_kl_POE = D.kl_divergence(uPOE, prior).sum(dim=1).mean() / x[net.keys[0]].shape[1]
            x_elbo_POE = {
                k: x_nll_POE[k] + self.lam_kl * x_kl_POE
                for k in net.keys
            }
            x_elbo_sum_POE = sum(self.modality_weight[k] * x_elbo_POE[k] for k in net.keys)
        else:
            x_elbo_sum_POE = torch.as_tensor(0.0, device=net.device)
        ...
        # Calculating orthogonality constraint loss using:
        # - usamp_mean_split: modality-specific embeddings for each sample based on taking
        #   the average across all output of the self- and cross-encoders; n_mod x batch_size x n_feat
        # - uPOEsamp: joint embedding

        if not self.lam_poe:
            uPOEsamp = usamp_mean_split.mean(axis=0)
            
        orthog_loss = torch.as_tensor(0.0, device=net.device)
        if self.lam_orthog:
            for i in range(n_mod):
                orthog_loss += torch.mean(torch.mul(uPOEsamp,usamp_mean_split[i]))

        # Contrastive loss between modalities - using average across all self- and cross-encoders
        if self.lam_contrastive:
            usamp_dict = {}
            for i in range(n_mod):
                usamp_dict[net.keys[i]] = usamp_mean_split[i]
            ctLoss = contrastive_loss(usamp_dict, pmsk_orig, net, self.temperature)
        else:
            ctLoss = torch.as_tensor(0.0, device=net.device)

        # Contrastive loss between samples
        if self.lam_contrast_samp:
            ctLoss_samp = contrast_samp(uPOEsamp_aug, net, self.temperature)
        else:
            ctLoss_samp = torch.as_tensor(0.0, device=net.device)
        ...
        if self.lam_klcross:
            x_klcross = {}
            for k in net.keys:
                x_klcross_temp = torch.as_tensor(0.0, device=net.device)
                for j in net.keys:
                    if k != j:
                        x_klcross_temp += D.kl_divergence(u[k], u[j]).sum(dim=1).mean()/x[k].shape[1]
                x_klcross[k] = x_klcross_temp
            klcross_loss = sum(x_klcross[k] for k in net.keys)
        else:
            klcross_loss = torch.as_tensor(0.0, device=net.device)
        ...
        gen_loss = self.lam_data * x_elbo_sum \
            ...
            + self.lam_klcross * klcross_loss \
            + self.lam_poe * x_elbo_sum_POE \
            + self.lam_contrastive * ctLoss \
            + self.lam_contrast_samp * ctLoss_samp \
            + self.lam_orthog * orthog_loss

        losses = {
            "dsc_loss": dsc_loss, "gen_loss": gen_loss,
            "x_elbo_sum": x_elbo_sum,
            "joint_cross_loss": joint_cross_loss,
            "real_cross_loss": real_cross_loss,
            "cos_loss": cos_loss,
            "klcross_loss": klcross_loss,
            "x_elbo_sum_POE": x_elbo_sum_POE,
            "ctLoss": ctLoss,
            "ctLoss_samp": ctLoss_samp,
...
@logged
class JASMINEModel(Model):

    NET_TYPE = SCCLUE
    TRAINER_TYPE = JASMINETrainer
    ...
    def adopt_pretrained_model(self, source: "JASMINEModel") -> None:
    ...
    def compile(
            ...
            lam_cos: float = 0.02,
            lam_klcross: float = 0.0,
            lam_poe: float = 1.0,
            lam_contrastive: float = 0.02,
            lam_contrast_samp: float = 0.02,
            temperature: float = 0.1,
            perturb_prop: float = 0.25,
            lam_orthog: float = 0.0,
        ...
        super().compile(
            ...
            lam_cos=lam_cos, normalize_u=normalize_u, modality_weight=modality_weight,latent_dim=latent_dim,
            lam_klcross=lam_klcross, lam_poe=lam_poe, lam_contrastive=lam_contrastive, 
            lam_contrast_samp=lam_contrast_samp, temperature=temperature, perturb_prop=perturb_prop,
    ...
    @torch.no_grad()
    def encode_sample(
            self, adatas: Mapping[str, AnnData], lam_poe, batch_size: int = 128
    ) -> np.ndarray:
        self.net.eval()
        data = AnnDataset(
            [adatas[key] for key in self.net.keys],
            [self.modalities[key] for key in self.net.keys],
            mode="train", getitem_size=batch_size
        )
        data_loader = DataLoader(
            data, batch_size=1,
            shuffle=False, drop_last=False
        ...
        for data in data_loader:
            x, xrep, xbch, xlbl, xdwt, xflag, pmsk = self.TRAINER_TYPE.format_data(self, data=list(data))
            xlbl_stack = torch.stack([xlbl[k] for k in self.net.keys])
            oripmsk = pmsk.T
            xlbl_mean = (xlbl_stack * oripmsk).sum(dim=0) / oripmsk.sum(dim=0)
            xlbl_mean = xlbl_mean.detach().cpu()
            labels.append(xlbl_mean)

            u, l  = {}, {}
            for k in self.net.keys:
                u[k], l[k] = self.net.x2u[k](x[k], xrep[k], lazy_normalizer=True)

            loc = u[self.net.keys[0]].mean
            latent_dim=int(loc.shape[-1]/len(self.net.keys))
            batch_size=loc.shape[0]

            if lam_poe:
                # PoE-derived joint distribution
                n_mod = len(self.net.keys)
                mu = torch.zeros(n_mod+1, batch_size, latent_dim, device=self.net.device)
                sigma = torch.ones(n_mod+1, batch_size, latent_dim, device=self.net.device)
                for i in range(0,len(self.net.keys)):
                    tempmean = u[self.net.keys[i]].mean # batch_size x latent_dim*n_mod
                    tempmean = tempmean.reshape(batch_size, latent_dim, n_mod) # batch_size x latent_dim x n_mod
                    tempmean = tempmean.permute(2,0,1) # n_mod x batch_size x latent_dim
                    tempmean = tempmean[i,:]
                    mu[i+1,:,:] = tempmean
    
                    tempsigma = u[self.net.keys[i]].stddev
                    tempsigma = tempsigma.reshape(batch_size, latent_dim, n_mod)
                    tempsigma = tempsigma.permute(2,0,1)
                    tempsigma = tempsigma[i,:]
                    sigma[i+1,:,:] = tempsigma
                ...
                uPOE = D.Normal(pd_mu, pd_std)
                # Sampling from joint distribution
                uPOEsamp = uPOE.rsample()
    
                result.append(uPOE.mean.detach().cpu())

            else:
                usamp = {k: u[k].mean for k in self.net.keys}

                pmsk = pmsk.T
                usamp_stack = torch.stack([usamp[k] for k in self.net.keys])
                pmsk_stack = pmsk.unsqueeze(2).expand_as(usamp_stack)
                usamp_mean = (usamp_stack * pmsk_stack).sum(dim=0) / pmsk_stack.sum(dim=0)

                xlbl_stack = torch.stack([xlbl[k] for k in self.net.keys])
                xlbl_mean = (xlbl_stack * pmsk).sum(dim=0) / pmsk.sum(dim=0)
                xlbl_mean = xlbl_mean.detach().cpu()

                result.append(usamp_mean.detach().cpu())
    
        return torch.cat(result).numpy(), torch.cat(labels).numpy()

    @torch.no_grad()
    def cross_encode_sample(
            self, adatas: Mapping[str, AnnData], normalize_u, batch_size: int = 128
    ) -> np.ndarray:
        self.net.eval()
        data = AnnDataset(
            [adatas[key] for key in self.net.keys],
        ...
        for data in data_loader:
            ...
            if normalize_u:
                usamp_mean = F.normalize(usamp_mean, dim=1)
            usamp_mean_split = torch.stack(torch.tensor_split(
                usamp_mean, len(self.net.keys), dim=1
            )) # n_mod x batch_size x n_feat
        ...
        return torch.cat(result, dim=0).numpy(), torch.cat(labels).numpy()
    ...
    def __repr__(self) -> str:
        return (
            f"JASMINE model with the following network and trainer:\n\n"
            f"{repr(self.net)}\n\n"
            f"{repr(self.trainer)}\n"
        )
